...
def getListForHistogramAndBoxPlot(df,spark):
    ...
    return data.rdd.map(lambda row : row.temperature).collect()
...
def getListsForRunChart(df,spark):
    df.createOrReplaceTempView("washing")
    data = spark.sql("SELECT temperature,ts from washing where temperature is not null order by ts")
    data_temp = data.rdd.sample(False , 0.1)
    ts = data_temp.map(lambda row:row.ts).collect()
    temperature = data_temp.map(lambda row:row.temperature).collect()
